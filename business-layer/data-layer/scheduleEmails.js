...
const { annualReportEmail, semesterReportEmail, statusReportEmail } = require("../business-logic-layer/public/emailTemplates");
const { Organization, EmailSettings, Membership, Member, Attendance } = require("../db");
    ...
    try {
        ...
        if (reportType === "statusReport") {
            ...
            for (const membership of members) {
                ...
                const numPointsAway = isActive ? null : Math.max(0, 48 - membership.member_points); // example threshold: 48
                const percent = isActive ? 100 : Math.round((membership.member_points / 48) * 100);
                ...
                // send email
                await sendEmail(member.member_email, emailData.subject, emailData.body);
            }
        } else {
            // handle semesterly and annual reports
            const totalMembers = await Membership.count({ where: { organization_id: orgId } });
            const activeMembers = await Membership.count({ where: { organization_id: orgId, active_member: true } });
            ...
            if (reportType === "annualReport") {
                emailData = annualReportEmail(orgAbr, meetingNum, eventNum, volunteerNum, activeMembers, totalMembers);
            } else if (reportType === "semesterlyReport") {
                emailData = semesterReportEmail(orgAbr, meetingNum, eventNum, volunteerNum, activeMembers, totalMembers);
            }
