...
describe('Attendance module', () => {
  ...
  describe('createAttendance', () => {
    ...
    it('should throw an error when creation fails', async () => {
      ...
      await expect(createAttendance(attendanceData)).rejects.toThrow('Database error');
      expect(console.error).toHaveBeenCalledWith('Error creating attendance:', mockError);
    });
  });
  ...
  describe('getAttendanceById', () => {
    ...
    it('should throw an error when fetch by id fails', async () => {
      ...
      // Call the function and expect it to throw
      await expect(getAttendanceById(attendanceId)).rejects.toThrow('Fetch by ID error');
      expect(console.error).toHaveBeenCalledWith('Error fetching attendance by ID:', mockError);
    });
  });
  ...
  describe('getAttendanceByMemberId', () => {
    ...
    it('should throw an error when fetch by member id fails', async () => {
      ...
      // Call the function and expect it to throw
      await expect(getAttendanceByMemberId(memberId)).rejects.toThrow('Fetch by member ID error');
      expect(console.error).toHaveBeenCalledWith('Error fetching attendance by member ID:', mockError);
    });
  });
  ...
  describe('getAttendanceByMemberAndEvent', () => {
    ...
    it('should throw an error when fetch by member and event fails', async () => {
      ...
      // Call the function and expect it to throw
      await expect(getAttendanceByMemberAndEvent(memberId, eventId)).rejects.toThrow('Fetch by member and event error');
      expect(console.error).toHaveBeenCalledWith('Error fetching attendance by member and event IDs:', mockError);
    });
  });
  ...
  describe('getMemberAttendanceWithEvents', () => {
    ...
    it('should throw an error when fetching member attendance with events fails', async () => {
      ...
      // Call the function and expect it to throw
      await expect(getMemberAttendanceWithEvents(orgId, memberId)).rejects.toThrow('Fetch error');
    });
  });
  ...
  describe('getMeetingAttendanceWithMembers', () => {
    ...
    it('should throw an error when fetching meeting attendance with members fails', async () => {
      // Mock data
      const meetingId = 20;
      const mockError = new Error('Fetch error');
      
      // Set up the mock to reject
      getMeetingAttendanceWithMembers.mockRejectedValue(mockError);
      
      // Call the function and expect it to throw
      await expect(getMeetingAttendanceWithMembers(meetingId)).rejects.toThrow('Fetch error');
    });
  });
});