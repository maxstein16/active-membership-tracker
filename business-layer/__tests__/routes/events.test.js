...
describe("Event module", () => {
  ...
  describe("getAttendanceByEventId", () => {
    ...
    it("should throw an error when fetch by event id fails", async () => {
      ...
      // Call the function and expect it to throw
      await expect(getAttendanceByEventId(eventId)).rejects.toThrow(
        "Fetch by event ID error"
      );
      expect(console.error).toHaveBeenCalledWith(
        "Error fetching attendance by event ID:",
        mockError
      );
    });
  });
  ...
  describe("getEventsByAttributes", () => {
    ...
    it("should throw an error when fetch by attributes fails", async () => {
      ...
      // Call the function and expect it to throw
      await expect(getEventsByAttributes(filters)).rejects.toThrow(
        "Fetch by attributes error"
      );
      expect(console.error).toHaveBeenCalledWith(
        "Error fetching events by attributes:",
        mockError
      );
    });
  });
  ...
  describe("getEventsWithAttendance", () => {
    ...
    it("should throw an error when fetching events with attendance fails", async () => {
      ...
      // Call the function and expect it to throw
      await expect(getEventsWithAttendance(orgId)).rejects.toThrow(
        "Fetch error"
      );
      expect(console.error).toHaveBeenCalledWith(
        "Error in getEventsWithAttendance:",
        mockError
      );
    });
  });
